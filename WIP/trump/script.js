...
function pickRandomItem(myArray) {
	myItem = myArray[Math.floor(Math.random()*myArray.length)]; 
	return myItem;
}
enemy = pickRandomItem(enemiesList);
insult = pickRandomItem(insultsList);
conclusion = pickRandomItem(conclusionsList);

function generateTweet(text){
  document.getElementById("tweet").innerHTML = enemiesList[Math.floor(Math.random()*enemiesList.length)] + " " + insultsList[Math.floor(Math.random()*insultsList.length)] + ". " + conclusionsList[Math.floor(Math.random()*conclusionsList.length)]}
